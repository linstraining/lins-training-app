...
const statusInfo = {
  ideal: { label: 'IDEAL', color: '#10b981', bg: '#d1fae5' },
  atencao: { label: 'ATENÇÃO', color: '#eab308', bg: '#fef3c7' },
  risco: { label: 'RISCO', color: '#ef4444', bg: '#fee2e2' },
...
const calcAssimetria = (dir, esq) => {
  ...
  let status;
  if (perc < 10) status = 'ideal';
  else if (perc <= 15) status = 'atencao';
  else status = 'risco';
...
export default function LinsTrainingApp() {
  ...
  const [pdfModulos, setPdfModulos] = useState({ anamnese: true, morfologico: true, neuromotor: true, postural: true });
  ...
  function gerarPDF(modulos) {
    ...
    const exercicios = [
      ...
      { nome: 'Extensão de Quadril', d: ev.forcaExtensaoQuadrilD, e: ev.forcaExtensaoQuadrilE, desc: 'Glúteos e posterior — potência de corrida e mudanças de direção.' },
      ...
      { nome: 'Pressão Manual', d: ev.forcaPressaoManualD, e: ev.forcaPressaoManualE, desc: 'Força de preensão — indicador global de força neuromuscular.' },
    ];
    ...
    const brandHeader = `
      <div class="header-brand">
        ...
        <div class="brand-text">
          ...
          <div class="brand-tagline"><div class="tag-main">PERFORMANCE</div><div class="tag-sub">HEALTH</div></div>
        </div>
        ...
        <div class="cover-top">
          <div class="cover-logo">
            ...
            <div class="cover-logo-text">
              ...
              <div class="cover-logo-tag"><div class="tag-main">PERFORMANCE</div><div class="tag-sub">HEALTH</div></div>
            </div>
          </div>
          <div class="cover-divider"></div>
          <div class="cover-title-block">
            <div class="cover-title-label">RELATÓRIO DE</div>
            <h1>AVALIAÇÃO DE PERFORMANCE</h1>
    ...
    const paginaMorfo = !modulos.morfologico ? '' : `
      <div class="page">
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr><td><strong>Braço</strong></td><td style="text-align:center;">${ev.bracoDir} cm</td><td style="text-align:center;">${ev.bracoEsq} cm</td><td style="text-align:center;">${Math.abs((ev.bracoDir||0) - (ev.bracoEsq||0)).toFixed(1)} cm</td></tr>
            <tr><td><strong>Coxa</strong></td><td style="text-align:center;">${ev.coxaDir} cm</td><td style="text-align:center;">${ev.coxaEsq} cm</td><td style="text-align:center;">${Math.abs((ev.coxaDir||0) - (ev.coxaEsq||0)).toFixed(1)} cm</td></tr>
    ...
    const paginaNeuro = !modulos.neuromotor ? '' : `
      <div class="page">
        ...
        <div class="criterios-row">
          <div class="criterio-card ideal-bg">
            <div class="criterio-label">IDEAL</div>
            <div class="criterio-value">&lt; 10%</div>
            <div class="criterio-desc">Equilíbrio funcional</div>
          </div>
          <div class="criterio-card atencao-bg">
            <div class="criterio-label">ATENÇÃO</div>
            <div class="criterio-value">10 – 15%</div>
            <div class="criterio-desc">Trabalho compensatório</div>
          </div>
          <div class="criterio-card risco-bg">
            <div class="criterio-label">RISCO</div>
            <div class="criterio-value">&gt; 15%</div>
            <div class="criterio-desc">Risco aumentado de lesão</div>
          </div>
        </div>
      ...
      <div class="page">
        ${brandHeader}
        <div class="title-section">
          <div class="title-label">MÓDULO 3 · CONTINUAÇÃO</div>
    ...
    const paginaEntenda = `
      <div class="page">
        ...
        <div class="title-section">
          ...
          <div class="title-sub">O que cada métrica significa pra sua performance</div>
        </div>
        ...
        <div class="entenda-grid">
          ...
          <div class="entenda-card">
            ...
            <p>Medida com dinamômetro. Critério Croisier: até 10% normal, 10-15% atenção, acima de 15% risco aumentado de lesão muscular. Padrão do futebol europeu.</p>
          </div>
          ...
          ${modulos.postural ? `
          <div class="entenda-card">
            <div class="entenda-title">Avaliação Postural</div>
            <p>Identifica desalinhamentos que afetam performance e saúde articular. Pequenos desvios podem gerar sobrecargas em cadeias musculares inteiras, aumentando risco de lesões.</p>
          </div>` : ''}
        </div>
        ...
        <div class="ref-box">
          ...
          <div class="ref-list">
            <div class="ref-item">Este relatório é um <strong>documento técnico de avaliação de performance</strong>.</div>
        ...
        <div class="signatures">
          <div class="sig-block">
            <div class="sig-line-h"></div>
            <div class="sig-title">AVALIADOR</div>
            <div class="sig-name-final">Lins Oliveira</div>
            <div class="sig-role-final">Lins Training</div>
          </div>
          <div class="sig-block">
            <div class="sig-line-h"></div>
            <div class="sig-title">ATLETA</div>
            <div class="sig-name-final">${ev.nome || '—'}</div>
            <div class="sig-role-final">${dataFmt}</div>
          </div>
        </div>
      </div>`;
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Helvetica', Arial, sans-serif; }
  ...
  /* CRITÉRIOS */
  .criterios-row { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px; }
  .criterio-card { padding: 12px; border-radius: 10px; text-align: center; border: 2px solid; }
  .criterio-card.ideal-bg { background: #d1fae5; border-color: #10b981; }
  .criterio-card.atencao-bg { background: #fef3c7; border-color: #eab308; }
  .criterio-card.risco-bg { background: #fee2e2; border-color: #ef4444; }
  .criterio-label { font-size: 9px; font-weight: 900; letter-spacing: 1.5px; }
  .ideal-bg .criterio-label { color: #065f46; }
  .atencao-bg .criterio-label { color: #78350f; }
  .risco-bg .criterio-label { color: #991b1b; }
  .criterio-value { font-size: 16px; font-weight: 900; margin-top: 4px; line-height: 1; }
  .ideal-bg .criterio-value { color: #065f46; }
  .atencao-bg .criterio-value { color: #78350f; }
  .risco-bg .criterio-value { color: #991b1b; }
  ...
  .ideal { background: #d1fae5; color: #065f46; }
  .atencao { background: #fef3c7; color: #78350f; }
  .risco { background: #fee2e2; color: #991b1b; }
...
${paginaEntenda}
    ...
    alert('HTML pronto! Tamanho: ' + html.length + ' chars');
    setReportHtml(html);
    alert('setReportHtml chamado. Aguarde a tela mudar.');
    } catch (err) {
      alert('Erro ao gerar PDF: ' + err.message + '\n\nLinha: ' + (err.lineNumber || 'N/A'));
  ...
  // ============ VISUALIZAÇÃO DO RELATÓRIO (iframe inline) ============
  if (reportHtml) {
    return (
      <div className="min-h-screen bg-slate-100 flex flex-col">
        <div className="bg-white border-b border-slate-200 sticky top-0 z-50 px-4 py-3 flex items-center justify-between gap-2">
          <button onClick={() => setReportHtml(null)} className="bg-slate-100 text-slate-700 px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2"
            >
        ...
        <div className="flex-1 bg-slate-200 p-2">
          <iframe
            ...
            style={{ minHeight: 'calc(100vh - 70px)', border: 'none' }}
            title="Relatório Lins Training"
          />
        </div>
      </div>
    );
      ...
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[10px] text-slate-500 font-medium tracking-wider mt-0.5">AVALIAÇÃO DE PERFORMANCE</div>
          </div>
        </div>
        {view !== 'home' && (
      ...
      <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-6xl mx-auto px-4 py-6 sm:py-10">
          <div className="mb-8">
            ...
            <p className="text-slate-500 text-sm mt-1">Sistema modular de avaliação de performance</p>
          </div>
  ...
  if (view === 'report') {
    const moduloItens = [
      ...
      { key: 'neuromotor', label: 'Neuromotor', desc: 'Força, RML e potência', icon: Zap, color: '#7c3aed' },
      { key: 'postural', label: 'Postural', desc: 'Fotos e parecer técnico', icon: Camera, color: '#db2777' },
    ];
  ...
  const modulos = [
    ...
    { num: 3, label: 'Neuromotor', icon: Zap, color: 'purple' },
    { num: 4, label: 'Postural', icon: Camera, color: 'pink' },
  ];
          ...
          <div className="space-y-4">
            ...
            <Card icon={Target} title="Perimetria" subtitle="11 medidas circunferenciais (cm)">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Input label="Coxa Direita" value={currentEvaluation.coxaDir} onChange={update('coxaDir')} unit="cm" />
                <Input label="Coxa Esquerda" value={currentEvaluation.coxaEsq} onChange={update('coxaEsq')} unit="cm" />
                ...
                <div className="mt-5 bg-slate-50 rounded-xl p-4 border border-slate-200">
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      { label: 'Coxa', d: currentEvaluation.coxaDir, e: currentEvaluation.coxaEsq },
          ...
          <div className="space-y-4">
            <div className="bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-100 rounded-2xl p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-black text-purple-900">Módulo 3 · Neuromotor</div>
                  <div className="text-xs text-purple-700">Força, resistência e potência</div>
                </div>
              </div>
            </div>
            ...
            <Card icon={Dumbbell} title="Avaliação de Força Muscular" subtitle="Análise de assimetrias D vs E (kg)">
              <div className="space-y-3">
                ...
                <ExercicioForca label="Extensão de Quadril" valD={currentEvaluation.forcaExtensaoQuadrilD} valE={currentEvaluation.forcaExtensaoQuadrilE} onChangeD={update('forcaExtensaoQuadrilD')} onChangeE={update('forcaExtensaoQuadrilE')} />
                ...
                <ExercicioForca label="Pressão Manual" valD={currentEvaluation.forcaPressaoManualD} valE={currentEvaluation.forcaPressaoManualE} onChangeD={update('forcaPressaoManualD')} onChangeE={update('forcaPressaoManualE')} />
              </div>
              ...
              <div className="mt-5 bg-blue-50 border-l-3 border-blue-500 rounded-r-lg p-3">
                ...
                <div className="text-xs text-blue-800 leading-relaxed">
                  Padrão do futebol europeu: <strong>abaixo de 10%</strong> = ideal · <strong>10–15%</strong> = atenção · <strong>acima de 15%</strong> = risco aumentado de lesão muscular.
                </div>
              </div>
            </Card>
          ...
          <div className="space-y-4">
            <div className="bg-gradient-to-r from-pink-50 to-rose-50 border border-pink-100 rounded-2xl p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-black text-pink-900">Módulo 4 · Postural</div>
                  <div className="text-xs text-pink-700">Análise visual e parecer técnico</div>
                </div>
              </div>
            </div>

            <Card icon={Camera} title="Fotos Posturais" subtitle="5 vistas do atleta — cada uma com posição específica">
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                <div>
                  <PhotoUpload label="Anterior · Braços ao lado" value={currentEvaluation.fotoFrenteBracosLado} onChange={update('fotoFrenteBracosLado')} />
                  <div className="text-[9px] text-slate-500 mt-1.5 text-center leading-tight">De frente, braços relaxados ao longo do corpo</div>
                </div>
                <div>
                  <PhotoUpload label="Anterior · Braços abertos" value={currentEvaluation.fotoFrenteBracosAbertos} onChange={update('fotoFrenteBracosAbertos')} />
                  <div className="text-[9px] text-slate-500 mt-1.5 text-center leading-tight">De frente, braços estendidos lateralmente</div>
                </div>
                <div>
                  <PhotoUpload label="Lateral Direita" value={currentEvaluation.fotoLateralD} onChange={update('fotoLateralD')} />
                  <div className="text-[9px] text-slate-500 mt-1.5 text-center leading-tight">Perfil direito, braços ao longo do corpo</div>
                </div>
                <div>
                  <PhotoUpload label="Lateral Esquerda" value={currentEvaluation.fotoLateralE} onChange={update('fotoLateralE')} />
                  <div className="text-[9px] text-slate-500 mt-1.5 text-center leading-tight">Perfil esquerdo, braços ao longo do corpo</div>
                </div>
                <div>
                  <PhotoUpload label="Posterior" value={currentEvaluation.fotoCostas} onChange={update('fotoCostas')} />
                  <div className="text-[9px] text-slate-500 mt-1.5 text-center leading-tight">De costas, braços relaxados</div>
                </div>
              </div>
              <div className="mt-4 bg-blue-50 border-l-3 border-blue-500 rounded-r-lg p-3">
                <div className="text-[10px] font-bold text-blue-900 uppercase tracking-wider mb-1">Padrão de fotografia</div>
                <div className="text-xs text-blue-800 leading-relaxed">
                  Atleta em pé com postura natural, sem forçar alinhamento. Roupa justa que permita visualizar marcos anatômicos. Iluminação uniforme e fundo neutro de preferência.
                </div>
              </div>
            </Card>

            <Card icon={FileText} title="Classificação & Parecer Final" subtitle="Avaliação postural geral e recomendações ao atleta">
              <div className="mb-4">
                <label className="block text-[10px] font-bold text-slate-600 uppercase tracking-wider mb-2">Classificação Postural Geral</label>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { value: 'Equilibrada', color: '#10b981', bg: '#d1fae5', desc: 'Sem alterações significativas' },
                    { value: 'Intermediária', color: '#eab308', bg: '#fef3c7', desc: 'Alterações pontuais' },
                    { value: 'Preocupante', color: '#ef4444', bg: '#fee2e2', desc: 'Alterações importantes' },
                  ].map((opt) => {
                    const active = currentEvaluation.classificacaoPostural === opt.value;
                    return (
                      <button
                        key={opt.value}
                        onClick={() => update('classificacaoPostural')(opt.value)}
                        className={`p-3 rounded-xl border-2 transition text-left ${active ? 'shadow-md' : 'border-slate-200 hover:border-slate-300 bg-white'}`}
                        style={active ? { background: opt.bg, borderColor: opt.color } : {}}
                      >
                        <div className="font-black text-sm" style={{ color: active ? opt.color : '#0f172a' }}>{opt.value}</div>
                        <div className="text-[10px] font-medium mt-0.5" style={{ color: active ? opt.color : '#64748b' }}>{opt.desc}</div>
                      </button>
                    );
                  })}
                </div>
              ...
              <TextArea
                label="Parecer & Recomendações ao Atleta"
                value={currentEvaluation.parecerFinal}
                onChange={update('parecerFinal')}
                placeholder="Recomendações práticas para o atleta com base na avaliação postural completa: trabalho corretivo, fortalecimento, alongamento, mobilidade, encaminhamentos..."
                rows={6}
              />
            </Card>
            ...
            <div className="bg-gradient-to-br from-indigo-600 to-purple-600 rounded-2xl p-5 text-white">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-black text-base">Avaliação Completa!</div>
                  <div className="text-sm opacity-90 mt-1">Você concluiu os 4 módulos. Salve a avaliação e na próxima etapa montaremos o gerador de PDF do atleta.</div>
                </div>
              </div>
            </div>
          </div>
